...
router.get('/jobs', (req, res) => {
  ...
  let sql = 'SELECT * FROM jobs WHERE tenant_id = ?';
...
router.get('/jobs/:id', (req, res) => {
  const row = orchestrator.getJob(req.params.id, getTenantId(req));
...
router.get('/jobs/:id/log', (req, res) => {
  if (!orchestrator.getJob(req.params.id, getTenantId(req))) return res.status(404).json({ error: 'not_found' });
  const db = getDb();
...
router.post('/mappings/:mappingId/jobs', (req, res, next) => {
  try {
    const db = getDb();
    // Tenant-scoped fetch, not a bare `WHERE id = ?` - this is what actually
    // guarantees a job can never be created from another tenant's mapping;
    // createJobFromMapping trusts mapping.tenant_id precisely because it's
    // only ever reached through a lookup like this one.
    const mapping = db.prepare('SELECT * FROM mappings WHERE id = ? AND tenant_id = ?').get(req.params.mappingId, getTenantId(req));
...
router.post('/jobs/bulk-create', (req, res, next) => {
  try {
    ...
    for (const mid of mappingIds) {
      const mapping = db.prepare('SELECT * FROM mappings WHERE id = ? AND tenant_id = ?').get(mid, tenantId);
      if (!mapping) continue;
